...
export const HeroCarousel = (props) => {
    ...
    return (
        ...
        {heroSections?.map( hero =>
            <div key={hero._id} className="Hero-carousel">
                <div className="Hero-carousel-h3">
                    <h3>{hero.title}</h3>
                </div>

                <div className="Hero-carousel-wrapper" style={{
                    width: `calc( ${hero.imagenes.length / 4} * 100%)`,
                    // gridTemplateColumns: `repeat(${hero.imagenes.length} , 15.625rem)`,
                    gridTemplateColumns: `repeat(${hero.imagenes.length} , 1fr)`,
                    transform: `translateX(-${page * 78.125}rem)`,
                    transition: `transform .4s ease`
                }}>

                    {hero.imagenes.map(imagen =>
                        <Heroimagenes key={imagen._id} {...imagen} />
                    )}
...
const Heroimagenes = (props) => {
    ...
    return (
        ...
        <img src={src} alt={alt} className='Hero-film-img' style={{ borderRadius: `0.625rem` }} />
